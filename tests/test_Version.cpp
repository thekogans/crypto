...
TEST (thekogans, Version) {
    const util::Version &version = crypto::GetVersion ();
    ...
    // of the crypto Version file
    //
    // This may seem silly but it actually verifies
    // the library is loadable and the build environment
    // is correctly configured
    CHECK_EQUAL (static_cast<util::ui32> (THEKOGANS_CRYPTO_MAJOR_VERSION), version.majorVersion);
    CHECK_EQUAL (static_cast<util::ui32> (THEKOGANS_CRYPTO_MINOR_VERSION), version.minorVersion);
    CHECK_EQUAL (static_cast<util::ui32> (THEKOGANS_CRYPTO_PATCH_VERSION), version.patchVersion);
}
...
TEST (thekogans, VersionToString) {
    ...
    #define VERSIONSTRING\
        stringize (THEKOGANS_CRYPTO_MAJOR_VERSION) "."\
        stringize (THEKOGANS_CRYPTO_MINOR_VERSION) "."\
        stringize (THEKOGANS_CRYPTO_PATCH_VERSION)
    CHECK_EQUAL (VERSIONSTRING, crypto::GetVersion ().ToString ().c_str ());
}
